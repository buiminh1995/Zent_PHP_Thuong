...
//Lấy ra 3 bài viết
$query_posts = "SELECT p.*, c.title as 'category' FROM posts 
p LEFT JOIN categories c ON p.category_id = c.id WHERE p.status= 1 AND p.category_id = ".$id." ORDER BY
...
//Lấy ra 5 bài viết
$query_5posts = "SELECT p.*, c.title as 'category' FROM posts 
p LEFT JOIN categories c ON p.category_id = c.id WHERE p.status= 1 AND p.category_id = ".$id." ORDER BY
